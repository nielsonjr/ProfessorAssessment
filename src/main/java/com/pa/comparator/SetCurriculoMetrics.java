...
//TODO
	public SetCurriculoResult calculateMetrics(Group group, Map<EnumPublicationLocalType, QualisData> qualisDataMap) {
		...
		for (Curriculo curriculo : group.getCurriculos()) {
			...
			concludedOrientations = concludedOrientations + result.getConcludedOrientations();
			onGoingOrientations = onGoingOrientations + result.getOnGoingOrientations();
		}
		
		int size = group.getCurriculos().size();
		mapQualis = this.calculateAllAverage(mapQualis, size);
